...
class BusinessIdeasChat:
    ...
    def __append_database(self,coords,prompt,elaborate):
        if len(coords)==1:
            self.__database[coords[0]]["prompt"]=prompt
            self.__database[coords[0]]["elaborate"]=elaborate
        else:
            self.__append_database(coords[1:],prompt,elaborate)
