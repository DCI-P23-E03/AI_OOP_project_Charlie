...
def create_prompt_2():
    ...
    prompt_level2=(
        ...
        +plans
        +"\nGive me please three best business ideas.write in the format: number. #name of the idea# $the area expert$"
    )
...
def create_prompt_4():
    ...
    l3response = data[5]['content'] # extract level 3 response from entire conversation
    #print(l3response)
    recog_pattern = "(\d+)\. #([^#]+)# - \$([^$]+)\$ - \*([^*]+)\*" #\n(.+?)(?=\n\d+\.|\Z)
...
def main():
    ...
    while True:
        ...
        expert=inc_prompt[0]
        conversation.append(
        {"role": "system", "content": (f"You are one of the best {expert} with 25 years of experience and in depth knowledge of business, markets, strategies, start-ups etc.write in the format: number. #name of the idea# $the area expert$ - |list in imperative form for the second person| do not write an introduction or a conclusion")}) # SYSTEM ROLE REASSIGN
        conversation.append({"role": "user", "content": prompt})
        ...
        expert=inc_prompt[0]
        conversation.append(
        {"role": "system", "content": (f"You are one of the best {expert} with 25 years of experience and in depth knowledge of business, markets, strategies, start-ups etc")})  # SYSTEM ROLE REASSIGN
